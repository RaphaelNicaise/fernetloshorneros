...
export function StepPayment({ items, shipping, coupon, total, onBack }: StepPaymentProps) {
  ...
  const onSubmit = async ({ formData }: any) => {
    ...
    try {
      ...
      console.log("[PaymentBrick] Respuesta process:", res.data)
      ...
      if (status === "approved") {
        ...
        const errorMessages: Record<string, string> = {
          ...
          cc_rejected_card_disabled: "La tarjeta no está habilitada. Llama a tu banco.",
          cc_rejected_max_attempts: "Llegaste al límite de intentos permitidos.",
        }
        setError(errorMessages[status_detail] || "El pago no pudo ser procesado. Intenta con otro medio de pago.")
        setIsProcessing(false)
      }
    } catch (err: any) {
      console.error("[PaymentBrick] Error procesando:", err)
  ...
  const onBrickError = (err: any) => {
    console.error("[PaymentBrick] Error del Brick:", err)
